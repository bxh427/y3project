...
double getArcLength(double THETA, double A, double B, Vector POS);
...
int main()
{
	...
	theta = getTheta(InitPos.ycomp,InitPos.xcomp,a,b);
	alpha = asin(-1*DotProduct(V_in,Norm)/sqrt(DotProduct(V_in,V_in)));
	p = cos(alpha);
...
Vector getVin(Vector v)
{
	if(v.ycomp >=0 && v.ycomp<100000 && v.xcomp>0)
	{
		v.ycomp = v.ycomp*10;
		v.xcomp = 10;
	}

	else if(v.ycomp >= 100000 && v.xcomp>0)
	{
		v.ycomp = 0.000001;
		v.xcomp = -10;
	}
	
	else if(v.ycomp>=0 && v.ycomp<100000 && v.xcomp<0)
	{
		v.ycomp = v.ycomp*10;
		v.xcomp = -10;
	}
...
double getArcLength(double THETA, double A, double B, Vector POS)
{
	...
	else if(THETA>0 && THETA<=atan2(A,B))
	{
		S = 0.5*A*atan2(POS.ycomp - 0.5*A, POS.xcomp - B);
	}

	else if(THETA == atan2(A,B))
	{
		S = A*pi*0.25;
	}
	...
	{
		S = 0.25*A*pi + B - POS.xcomp;
	}

	else if(THETA == atan2(A,B) + 0.5*pi)
	{
		S = 0.25*A*pi + B;
	}
	...
	{
		S = 0.25*A*pi + B + 0.5*A*atan2(POS.xcomp,0.5*A - POS.ycomp);
	}
	
	else if(THETA == -0.5*pi - atan2(A,B))
	{
		S = 0.75*A*pi + B; 
	}
	...
	{
		S = 0.75*A*pi + 2*B + 0.5*A*atan2(0.5*A - POS.ycomp, POS.xcomp - B);
	}
